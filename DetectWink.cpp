...
*/
string OPENCV_ROOT = "C:/Ceemple/OpenCV4VS/";
string cascades = OPENCV_ROOT + "samples/data/haarcascades/";
...
bool detectWink(Mat frame, Point location, Mat ROI, CascadeClassifier cascade) {
  // frame,ctr are only used for drawing the detected eyes
    vector<Rect> eyes;
	cascade.detectMultiScale(ROI, eyes, 1.1, 15, 0 | CASCADE_DO_ROUGH_SEARCH | CASCADE_SCALE_IMAGE, Size(30, 20), Size(100, 40));
...
int detect(Mat frame, 
  ...
  Mat  smallImg(cvRound(frame.rows / 1), cvRound(frame.cols / 1), CV_8UC1);
...
/*  input,output,neighborood_size,center_location (neg means - true center) */
  cascade_face.detectMultiScale(smallImg, faces,
	  1.1, 3, 0 | CASCADE_DO_ROUGH_SEARCH | CASCADE_SCALE_IMAGE, Size(30, 30));
